...
function counter() {
    ...
    var timer = setInterval(function() {
        ...
        setTimeout(() => {
            if (counter > -1) {
                $('#countdown').css({ 'font-size': '20vw', 'opacity': 0, 'font-weight': 'bold', 'z-index': '10' });
            } else {
                $('#countdown').css({ 'font-size': '20vw', 'opacity': 50, 'font-weight': 'bold', 'z-index': '10' });
            }
        }, 20);
        counter--;
...
function init() {
    ...
    var light = new THREE.HemisphereLight(0xeeeeff, 0x000000, 0.75);
    light.position.set(0, 100, 0.4);
    scene.add(light);

    var dirLight = new THREE.SpotLight(0x000000, .5, 0.0, 180.0);
    ...
    for (var i = 0; i < 500; i++) {
        var color;
        color == 0xff1493 ? color = 0x800080 : color = 0xff1493;

        var boxMaterial = new THREE.MeshStandardMaterial({ color: color, flatShading: false, vertexColors: false });
...
function makeParticles(intersectPosition) {
    ...
    for (var i = 0; i < totalParticles; i++) {
        ...
        var color = new THREE.Color(Math.random() * 0x000000);
        colors.push(color);
    }
